                                       ...
                                       dt_masks_form_gui_t *gui, int index)
{
  ...
  if(gui->group_edited == index || gui->point_selected >= 0 || gui->feather_selected >= 0 || gui->seg_selected >= 0
...
static void _path_events_post_expose(cairo_t *cr, float zoom_scale, dt_masks_form_gui_t *gui, int index, int nb)
{
  ...
  if(gui->group_edited == index && gpt->points_count > nb * 3 + 6)
  {
    for(int k = 0; k < nb; k++)
    {
  ...
  if((gui->group_edited == index) && gpt->border_count > nb * 3 + 6)
  {
    int dep = 1;
